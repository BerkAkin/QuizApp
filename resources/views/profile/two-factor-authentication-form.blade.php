<x-jet-action-section>
    ...
    <x-slot name="description">
        {{ __('İki adımlı kimlik doğrulamayı kullanarak hesabınıza ek güvenlik ekleyin.') }}
    </x-slot>

    <x-slot name="content">
        <h3 class="text-lg font-medium text-gray-900">
            @if ($this->enabled)
                {{ __('İki admılı kimlik doğrulamayı etkinleştirdiniz.') }}
            @else
                {{ __('İki admılı kimlik doğrulamayı etkinleştirmediniz.') }}
            @endif
        </h3>

        <div class="mt-3 max-w-xl text-sm text-gray-600">
            <p>
                {{ __('İki faktörlü kimlik doğrulama etkinleştirildiğinde, kimlik doğrulama sırasında sizden güvenli, rastgele bir belirteç istenir. Bu belirteci telefonunuzun Google Authenticator uygulamasından alabilirsiniz.') }}
            </p>
        </div>
